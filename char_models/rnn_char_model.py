...
## main loop
def train():
    global p,n,max_iter,mWxh,mWhh,mWhy,mbh,mby,smooth_loss
...
def plot_loss(max_iter,loss_vals):
    plt.plot(range(max_iter),loss_vals)
    plt.show()

if __name__ == "__main__":
    h = train()

    # plot_loss(max_iter,loss_vals)

    sample_ix = sample(h, char_to_ix[data[100]], 2000)
    txt = ''.join(ix_to_char[ix] for ix in sample_ix)
    print '---- sample -----'
    print '----\n %s \n----' % (txt, )
